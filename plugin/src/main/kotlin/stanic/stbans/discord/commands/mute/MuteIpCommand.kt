...
fun JDA.registerMuteIpCommand() = command("muteip") {
    if (!PermissionUtil.checkPermission(member, Permission.KICK_MEMBERS)) channel.reply("noPermDiscord".get().replace("{mention}", author.asMention))
    else {
        val args = message.contentRaw.split(" ")
        if (args.size < 2) {
            channel.reply("usageMuteIpDiscord".get().replace("{mention}", author.asMention))
            return@command
        }
